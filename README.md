...
**News** is a full-featured, high-performance iOS news application built with modern SwiftUI, SwiftData, Swift Concurrency, Metal Shaders, and ActivityKit. Designed with a custom glassmorphism design system, instant runtime multi-language localization, and comprehensive Unit and UI test coverage.

---

## 🔥 Features & Highlights

- **⚡ Native Swift Concurrency**: 100% pure Swift Concurrency (`async/await`, `@Observable`, `@MainActor`, `Task`, `Sendable`). `Combine` has been completely removed from the codebase.
...
- **🧪 Full Automated Test Suite**:
  - **Unit Testing**: Powered by Swift Testing (`import Testing` `@Test`) for ViewModels (`MainViewModel`, `SettingsViewModel`), `SettingsManager`, async network pipelines, models, extensions, and localization logic.
...
- **📌 Offline Favorites & Storage**: SwiftData persistence for offline article reading and read/unread status tracking with `@MainActor` thread-safe `SettingsManager`.
...
| :--- | :--- |
| **Architecture** | MVVM + `@Observable` + Protocol-Oriented Programming (POP) + `ModuleBuilder` DI |
...
| **Concurrency** | Pure Swift Concurrency (`async/await`, `@MainActor`, `Sendable`, `NSKeyValueObservation`) |
...
├── Modules/        # Main, Details, Favorites & Settings (MVVM + @Observable)
├── Managers/       # SettingsManager (@MainActor), NetworkManager, WidgetsManager, NotificationManager...
